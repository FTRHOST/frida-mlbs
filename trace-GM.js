...
function hookMethodReturnBool(targetClassName, targetMethodName, forceReturn) {
  if (!il2cpp) {
    console.log("[!] Error: Modul il2cpp tidak ditemukan.");
    return;
  }
  ...
  let iter = Memory.alloc(Process.pointerSize).writePointer(NULL);
  let methodPtr;
  let isFound = false; // Variabel penanda apakah method ditemukan

  while (!(methodPtr = il2cpp.class_get_methods(klassPtr, iter)).isNull()) {
    if (il2cpp.method_get_name(methodPtr).readUtf8String() === targetMethodName) {
      isFound = true; // Method berhasil ditemukan
      ...
      if (!impl.isNull()) {
        ...
        console.log(`[+] Sukses: Method '${targetMethodName}' di class '${targetClassName}' ditemukan dan berhasil di-hook (Force Return: ${forceReturn}).`);
      } else {
        console.log(`[!] Peringatan: Method '${targetMethodName}' di class '${targetClassName}' ditemukan, tetapi implementasinya NULL (tidak bisa di-hook).`);
      }
      break; // Keluar dari loop karena method sudah ketemu
    }
  }

  // Jika setelah loop selesai method tetap tidak ditemukan
  if (!isFound) {
    console.log(`[-] Gagal: Method '${targetMethodName}' tidak ditemukan di dalam class '${targetClassName}'.`);
  }
}
...
function dumpSkinTable() {
  if (!il2cpp || !il2cpp.runtime_invoke) return console.log("[!] API IL2CPP belum siap!");

  console.log("\n=================================================");
  console.log("[*] MEMULAI OPERASI EKSTRAKSI TABEL SKIN");
  console.log("=================================================");

  // 1. Cari Class CData_HeroCostume
  let targetClass = "CData_HeroCostume";
  let klassPtr = findClassPtr(targetClass);
  if (!klassPtr) return console.log(`[!] Class ${targetClass} tidak ditemukan.`);

  // 2. Cari Method GetInstance() secara statis
  let iter = Memory.alloc(Process.pointerSize).writePointer(NULL);
  let methodPtr = NULL;
  let getInstanceMethod = NULL;

  while (!(methodPtr = il2cpp.class_get_methods(klassPtr, iter)).isNull()) {
    let mName = il2cpp.method_get_name(methodPtr).readUtf8String();
    if (mName === "GetInstance") {
      getInstanceMethod = methodPtr;
      break;
    }
  }

  if (!getInstanceMethod) return console.log("[!] Method GetInstance() tidak ditemukan.");

  // 3. Eksekusi GetInstance() untuk mengambil pointer instance utamanya
  let exception = Memory.alloc(Process.pointerSize).writePointer(NULL);
  let instancePtr = il2cpp.runtime_invoke(getInstanceMethod, NULL, NULL, exception);

  if (instancePtr.isNull() || !exception.readPointer().isNull()) {
    return console.log("[!] Gagal mengambil Instance CData_HeroCostume.");
  }
  console.log(`[+] CData_HeroCostume Instance : ${instancePtr}`);

  // 4. Cari Method GetAll()
  iter.writePointer(NULL);
  let getAllMethod = NULL;
  while (!(methodPtr = il2cpp.class_get_methods(klassPtr, iter)).isNull()) {
    let mName = il2cpp.method_get_name(methodPtr).readUtf8String();
    if (mName === "GetAll") {
      getAllMethod = methodPtr;
      break;
    }
  }

  if (!getAllMethod) return console.log("[!] Method GetAll() tidak ditemukan.");

  // 5. Eksekusi GetAll() untuk mendapatkan pointer ke struktur SharedDictionary
  let dictPtr = il2cpp.runtime_invoke(getAllMethod, instancePtr, NULL, exception);
  if (dictPtr.isNull() || !exception.readPointer().isNull()) {
    return console.log("[!] Gagal mengekstrak Data Dictionary dari GetAll().");
  }
  console.log(`[+] SharedDictionary Pointer   : ${dictPtr}`);

  // =========================================================
  // 6. HEX DUMP MEMORY SHARED DICTIONARY
  // =========================================================
  console.log("\n[*] Melakukan Hex Dump pada memori SharedDictionary (Melihat struktur mentah)...");
  try {
    // Kita baca 64 bytes pertama dari SharedDictionary
    let buf = dictPtr.readByteArray(64);
    ...
    console.log(`[!] Gagal melakukan Hex Dump: ${err}`);
  }

  // =========================================================
  // 7. MENCARI METODE AKSES BAWAAN (Lebih Aman dari Raw Scan)
  // =========================================================
  console.log("\n[*] Mengintip method yang tersedia di SharedDictionary:");
  let dictClass = dictPtr.readPointer(); // Pointer ke metadata class
  let iterDict = Memory.alloc(Process.pointerSize).writePointer(NULL);
  let methodPtrDict = NULL;

  while (!(methodPtrDict = il2cpp.class_get_methods(dictClass, iterDict)).isNull()) {
    let mName = il2cpp.method_get_name(methodPtrDict).readUtf8String();
    let paramCount = il2cpp.method_get_param_count(methodPtrDict);
    // Filter method yang berguna untuk mengambil data
    if (mName.includes("get_") || mName.includes("Get") || mName.includes("Count") || mName.includes("Item")) {
      console.log(`    -> [Dict API] ${mName} (Params: ${paramCount})`);
    }
  }

  console.log("\n[*] Mengintip method yang tersedia di CData_HeroCostume (TableStream):");
  let iterTable = Memory.alloc(Process.pointerSize).writePointer(NULL);
  let methodPtrTable = NULL;

  while (!(methodPtrTable = il2cpp.class_get_methods(klassPtr, iterTable)).isNull()) {
    let mName = il2cpp.method_get_name(methodPtrTable).readUtf8String();
    let paramCount = il2cpp.method_get_param_count(methodPtrTable);
    // Filter method pencarian data dari table manager
    if (mName.includes("Get") || mName.includes("Count") || mName.includes("Find") || mName.includes("Item")) {
      console.log(`    -> [Table API] ${mName} (Params: ${paramCount})`);
    }
  }
}

// =========================================================
// CUSTOM FITUR: GAMEOBJECT & PERMISSION TRACKER
// =========================================================

function trackGameObjectStatus(targetObjName) {
  if (!il2cpp) return console.log("[!] IL2CPP belum siap!");
  console.log(`\n[*] Memulai Pelacakan Kehidupan GameObject: "${targetObjName}"...`);

  let goKlass = findClassPtr("GameObject"); // Cari class GameObject
  let objKlass = findClassPtr("Object", "UnityEngine");

  if (!goKlass || !objKlass) return console.log("[!] Gagal menemukan class dasar UnityEngine.");

  // Kita cari manual method SetActive dan get_name
  let iter = Memory.alloc(Process.pointerSize).writePointer(NULL);
  let methodPtr;
  let setActiveImpl = null;
  let getNameImpl = null;

  // Cari SetActive
  while (!(methodPtr = il2cpp.class_get_methods(goKlass, iter)).isNull()) {
    if (il2cpp.method_get_name(methodPtr).readUtf8String() === "SetActive") {
      setActiveImpl = methodPtr.readPointer();
      break;
    }
  }
  ...
  // Cari get_name
  iter.writePointer(NULL);
  while (!(methodPtr = il2cpp.class_get_methods(objKlass, iter)).isNull()) {
    if (il2cpp.method_get_name(methodPtr).readUtf8String() === "get_name") {
      getNameImpl = methodPtr.readPointer();
      break;
    }
  }

  if (setActiveImpl && getNameImpl && !setActiveImpl.isNull()) {
    let GetNameFunc = new NativeFunction(getNameImpl, 'pointer', ['pointer']);

    Interceptor.attach(setActiveImpl, {
      onEnter: function(args) {
        let goPtr = args[0];
        let state = args[1].toInt32(); // 1 = True (Muncul), 0 = False (Sembunyi)

        if (!goPtr.isNull()) {
          try {
            let namePtr = GetNameFunc(goPtr);
            let name = readIl2CppStringSafe(namePtr);

            if (name && name.toLowerCase().includes(targetObjName.toLowerCase())) {
              console.log(`\n=========================================`);
              console.log(`[🎯 TARGET TERDETEKSI] GameObject: ${name}`);
              console.log(`[-] Status saat ini diubah menjadi: ${state === 1 ? "MUNIK / TRUE" : "SEMBUNYI / FALSE"}`);

              // Jika game mencoba menyembunyikan, kita PAKSA MUNCUL!
              if (state === 0) {
                console.log(`[!] Game mencoba menyembunyikan tombol. MEMAKSA MUNCUL (True)!`);
                args[1] = ptr(1);
              }

              // Melacak Class apa yang mencoba menyembunyikan/memunculkan tombol ini
              console.log(`[*] Jejak Stack (Siapa yang memanggil ini?):`);
    ...
    console.log(`[+] GameObject Tracker aktif untuk mendeteksi: ${targetObjName}`);
  }
}

// =========================================================
// CUSTOM FITUR: GHOST CLICKER (FORCE CLICK VIA MEMORY)
// =========================================================

function forceClickGameObject(targetName) {
  if (!il2cpp || !il2cpp.string_new) return console.log("[!] API IL2CPP belum siap!");
  il2cpp.thread_attach(il2cpp.domain_get());

  console.log(`\n[*] Mencari GameObject: "${targetName}" di dalam memori...`);

  let goKlass = findClassPtr("GameObject", "UnityEngine");
  if (!goKlass) return console.log("[!] Class GameObject tidak ditemukan.");

  // Mencari method GameObject.Find(string) dan GameObject.SendMessage(string)
  let iter = Memory.alloc(Process.pointerSize).writePointer(NULL);
  let methodPtr;
  let findImpl = null;
  let sendMsgImpl = null;

  while (!(methodPtr = il2cpp.class_get_methods(goKlass, iter)).isNull()) {
    let mName = il2cpp.method_get_name(methodPtr).readUtf8String();
    let paramCount = il2cpp.method_get_param_count(methodPtr);

    if (mName === "Find" && paramCount === 1) findImpl = methodPtr.readPointer();
    if (mName === "SendMessage" && paramCount === 1) sendMsgImpl = methodPtr.readPointer();
  }

  if (findImpl && sendMsgImpl) {
    let FindFunc = new NativeFunction(findImpl, 'pointer', ['pointer']);
    let SendMsgFunc = new NativeFunction(sendMsgImpl, 'void', ['pointer', 'pointer']);

    try {
      // 1. Cari GameObject-nya
      let targetStrPtr = il2cpp.string_new(Memory.allocUtf8String(targetName));
      let goPtr = FindFunc(targetStrPtr);

      if (goPtr.isNull() || goPtr.toInt32() === 0) {
        console.log(`[!] GAGAL: GameObject "${targetName}" tidak ditemukan di RAM.`);
        console.log(`[-] Ini membuktikan Menu UI_GM memang belum di-instantiate oleh game.`);
      } else {
        console.log(`[+] TARGET DITEMUKAN! Alamat memori: ${goPtr}`);
        console.log(`[*] Mengirimkan sinyal klik palsu (SendMessage("OnClick"))...`);

        // 2. Kirim sinyal klik paksa
        let clickEventPtr = il2cpp.string_new(Memory.allocUtf8String("OnClick"));
        SendMsgFunc(goPtr, clickEventPtr);

        console.log(`[🚀] BERHASIL! Tombol "${targetName}" berhasil ditekan secara gaib!`);
      }
    } catch (e) {
      console.log(`[!] Error saat force click: ${e}`);
    }
  } else {
    console.log("[!] Gagal meload API Find/SendMessage dari Unity.");
  }
}

// =========================================================
// CUSTOM FITUR: THE INITIATOR (FORCE UI INITIALIZATION)
// =========================================================

function forceInitUI(uiClassName, initMethodName) {
  if (!il2cpp) return console.log("[!] IL2CPP belum siap!");

  let klassPtr = findClassPtr(uiClassName);
  if (!klassPtr) return console.log(`[!] Class UI ${uiClassName} tidak ditemukan.`);

  console.log(`\n[*] Memasang perangkap untuk Class UI: ${uiClassName}...`);
  console.log(`[*] Silakan buka/klik menu GM Anda di dalam game sekarang!`);

  // Kita memantau setiap method di class ini. 
  // Saat frame UI-nya disentuh/dibuka, kita bajak instance-nya dan paksa jalankan Init!
  hookAllMethods(klassPtr, (methodName, args) => {
    let instance = args[0];

    // Pastikan instance valid, dan kita belum memaksanya
    if (instance && !instance.isNull() && !lastFieldValues["ui_inited_" + uiClassName]) {
      console.log(`\n[+] Wadah UI (${uiClassName}) terdeteksi di memori: ${instance}`);

      // Jeda sedikit agar wadah selesai dirender, lalu paksa isi!
      setTimeout(() => {
        console.log(`[🚀] MEMAKSA INISIASI TOMBOL: Memanggil ${uiClassName}::${initMethodName}()`);
        invokeMethod(instance, uiClassName, initMethodName, []);
      }, 500);

      lastFieldValues["ui_inited_" + uiClassName] = true;
    }
  });
}
