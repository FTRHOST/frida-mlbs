...
/**
 * MLBB Frame Swapper: Mengganti Frame Rank menjadi Contest
 */

// === KONFIGURASI ===
// Jika saat Anda klik menu Rank ternyata nama frame yang muncul berbeda di log, ganti string di bawah ini:
const TARGET_FRAME = "FRAME_TeamMatchLua"; // Secara default akan mencari yang ada kata "Rank"
const REPLACE_WITH = "FRAME_ContestMatch"; // Frame pengganti
...
const check = setInterval(() => {
    ...
    if (mod) { 
        ...
        if (frameIdClass) {
            // Menginisialisasi Class agar nilai-nilai variabel statisnya dimuat ke memory
            ...
            // Loop ke semua Frame yang ada untuk mencari alamat pointer-nya
            while (!(fieldPtr = il2cpp.class_get_fields(frameIdClass, iter)).isNull()) {
                ...
                if (fieldName.startsWith("FRAME_") || fieldName.startsWith("UI_")) {
                    try {
                        ...
                        if (!objPtr.isNull()) {
                            frameMap[objPtr.toString()] = fieldName; // Mapping Pointer -> String
                            count++;

                            // Simpan spesifik alamat memory dari FRAME_Contest
            ...
            if (replaceFramePtr) {
                console.log(`[+] Pointer Pengganti (${REPLACE_WITH}) ditemukan: ${replaceFramePtr}`);
            } else {
                console.log(`[-] Peringatan: Pointer ${REPLACE_WITH} tidak ditemukan / null.`);
            }

        } else {
            console.log("[-] GAGAL: Class FrameID tidak ditemukan.");
            return;
        }
        ...
        if (uiMgrClass) {
            ...
            hookAllMethods(uiMgrClass, (methodName, args) => {
                // Method 'Active' digunakan saat game membuka halaman Frame baru
                if (methodName === "Active") {
                    try {
                        let framePtr = args[1]; // Membaca pointer dari argumen pertama (FrameID)
                        let frameName = frameMap[framePtr.toString()]; // Mendapatkan nama String-nya
                        
                        if (frameName) {
                            console.log(`\n[UI EVENT] Anda membuka: ${frameName}`);

                            // ==== LOGIKA HOOK SWAPPING ====
                            // Deteksi jika Anda menekan menu Rank (atau frame yang mengandung nama "Rank")
                            if (frameName === TARGET_FRAME || frameName.includes("Rank")) {
                                console.log(`   [!] Terdeteksi masuk ke menu Rank!`);
                                
                                if (replaceFramePtr !== null) {
                                    console.log(`   [->] MENGALALIHKAN (HOOK) KE: ${REPLACE_WITH}`);
                                    
                                    // INTI DARI PATCHING: Menimpa Argumen ke-1 dengan Pointer FRAME_Contest
